...
const VideoView = () => {
    ...
    useEffect(() => {
        const fetchVideo = async () => {
            ...
            try {
                ...
                const { data } = await axios.get(`${API_URL}/api/videos/${id}?s=${shareId || ''}`, config);
                setVideo(data);
    ...
    const openCallModal = async () => {
        ...
        setCallRequestData({ name: '', phone: '', email: '' });
        setShowCallModal(true);

        if (shareId) {
            setCallInfoLoading(true);
            try {
                const { data } = await axios.get(`${API_URL}/api/contact/customer-info/${shareId}`);
                // Normalise phone for UKPhoneInput: strip +44 or leading 0, keep local digits only
                let rawPhone = data.phone || '';
                rawPhone = rawPhone.replace(/\s/g, '');
                if (rawPhone.startsWith('+44')) rawPhone = rawPhone.slice(3);
                ...
                console.log('Could not pre-fill customer info:', err);
            } finally {
        ...
        try {
            await axios.post(`${API_URL}/api/contact/request-call`, {
                ...
                phone: callRequestData.phone
                    ? `+44${callRequestData.phone.replace(/^\+44/, '').replace(/^0/, '')}`
                    : '',
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4 text-center">
                ...
                <p className="text-gray-500 max-w-md">
                    {error === 'expired'
                        ? 'For security reasons, this video walkthrough link has expired (4-day limit). Please contact Heston Automotive to request a new link.'
                        : error === 'suspended'
                        ? 'Access to this video link has been temporarily suspended by the dealer. Please contact Heston Automotive for assistance.'
                        : 'The video link you are trying to access is no longer available or is invalid.'}
                </p>
                <div className="mt-8 flex flex-col gap-3 w-full max-w-xs">
                    <a href="tel:02085648030" className="bg-blue-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-blue-700 transition shadow-lg">
                        Call 020 8564 8030
                    </a>
                    <a href="mailto:enquiries@hestonautomotive.com" className="bg-white text-gray-700 border border-gray-200 px-6 py-3 rounded-lg font-bold hover:bg-gray-50 transition">
                        Email Us
                    </a>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full animate-fade-in">
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button
                                    onClick={() => { setShowCallModal(false); setCallSuccess(false); }}
                                    className="text-gray-400 hover:text-gray-600 text-2xl"
                                >
                                    ×
                                </button>
                            ...
                            {callSuccess ? (
                                <div className="text-center py-6">
                                    ...
                                    <p className="text-gray-500 text-sm">We'll call you back as soon as possible.</p>
                                    <button
                                        onClick={() => { setShowCallModal(false); setCallSuccess(false); }}
                                        className="mt-5 w-full py-2 bg-gray-900 text-white rounded-lg font-semibold hover:bg-gray-800 transition-colors"
                                    >
                                        Close
                                    </button>
                                </div>
                                ...
                                <form onSubmit={handleCallRequest} className="space-y-4">
                                    {shareId && (
                                        <p className="text-xs text-blue-600 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                                            ✏️ Your details are pre-filled. Please review and edit if needed before confirming.
                                        </p>
                                    )}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="email"
                                            value={callRequestData.email}
                                            onChange={(e) => setCallRequestData({ ...callRequestData, email: e.target.value })}
                                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    ...
                                    <div className="pt-2">
                                        ...
                                        >
                                            {callLoading ? 'Sending Request...' : '📞 Confirm Call Back'}
                                        </button>
                                    </div>
                                </form>
                            )}
